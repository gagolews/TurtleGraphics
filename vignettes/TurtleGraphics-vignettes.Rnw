...
\paragraph{\textsf{turtle\_forward} and \paragraph{\textsf{turtle\_backward}}}
...
\paragraph{\textsf{turtle\_right} and \paragraph{\textsf{turtle\_left}}}
...
\paragraph{\textsf{turtle\_up} and \paragraph{\textsf{turtle\_down}}}
...
\paragraph{\textsf{turtle\_hide} and \paragraph{\textsf{turtle\_show}}}
...
\paragraph{\textsf{turtle\_col}, \textsf{turtle\_lty}  and \paragraph{\textsf{turtle\_lwd}}}
...
\paragraph{\textsf{turtle\_status}, \textsf{turtle\_getpos}  and \paragraph{\textsf{turtle\_getangle}}}
...
This section shows how to connect the functions listed above with the options that \textsf{R} gives us. For example, sometimes you would like to repeat some actions several times. If it is once or twice then it is easy to type it. But if you wish to use the same functions for, let's say, 360 times then it turns out to be quite difficult. 

Do not worry though! \textsf{R} has the tools to deal with it. In the case you want to repeat actions several times we can use the so called loop. The syntax is the following: \textsf{for(i in 1:100)\{\}}. \textsf{i} is the counter that counts the turns of the loop. \textsf{1:100} defines when the counter should start counting and when it should stop. In this case the counter \textsf{i} starts in 1 and it increases by 1 each time the loop turns. The loop will stop turning when \textsf{i} reaches 100.

Between the curly rackets (\textsf{\{\}}) you write the functions you would like to repeat. For example:
...
Sometimes it takes time to make the loop, especially when it turns many times. As you can see by using the loop a circle was drawn.
...
turtle_forward(dist=15)
@
As you can the condition is fulfilled so the Turtle turn right. What if you would like to perform some actions when the condition is fulfilled and some other actions in case it is not? There is an answer to it too. After the closing curly bracket you write \textsf{else \{\}} and again between the brackets there are the functions you would like to call.
<<results=hide, echo=FALSE>>=
...
\textsf{square} is the name of the function. After the word \textsf{function} you list the parameters in brackets. If there is more than one parameter you list them separating by comma. You can make the parameters default as in the example: \textsf{r=10} but you can also leave it to the decision of the user. Then it would be \textsf{function(r)}. You have to compile the function and then you can use it as all the other functions:
<<results=hide, echo=FALSE>>=
...
The other example of recursion is a fractal. Usually, a fractal is an image which at every scale shows the same pattern. In the example section (\ref{ex}) you have 3 characteristic examples of fractals -- the fractal tree, the Koch snowflake and the Sierpinski triangle.
...
% \newpage
% 
% \subsubsection{The Turtle Rainbow Star}
% <<results=hide, echo=FALSE>>=
...
% \newpage
% 
% \subsubsection{The Turtle Brownian}
...
% turtle_do(turtle_brownian(500, length=25))
% dev.off()  
% @
% 
% <<results=tex, echo=FALSE>>=
...
% \newpage
% 
% \subsubsection{The Fractal Tree}
% <<results=hide, echo=FALSE>>=
