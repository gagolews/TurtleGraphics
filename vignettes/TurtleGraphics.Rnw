...
Then you load the package with the \textsf{require()} function, as it is shown below.
...
You can easily change it by passing as the arguments \textsf{width}
and \textsf{height} (e.g. \textsf{turtle\_init(width=200, height=200)}).
...
on the other side of the board in case of crossing its border.

\paragraph{\textsf{turtle\_forward} and \textsf{turtle\_backward}.}

There are two main groups of functions used to move the Turtle.

The first one consists of the \textsf{turtle\_forward()}
and the \textsf{turtle\_backward()} functions.
In its argument you have to give the distance you desire the Turtle to move.
...
function with the negative number as an argument or simply
...
\textsf{turtle\_left} and the \textsf{turtle\_right} change the Turtle's direction by a given angle.
...
Let's discuss some additional features that you can play with.
...
Similarly, you may show or hide the Turtle image,
using the \textsf{turtle\_show()} and \textsf{turtle\_hide()} functions,
respectively. If you are calling a lot of functions it is strongly
recommended to hide the Turtle first as it speeds up the drawing process,
...
The first one, as you can easily guess, changes the color
of the path. For example, if you wish to change the trace into green, try:
...
the type and the width of the line.
To change the type of the path pass as an argument a number from $0$ to $6$
-- each denotes a different type of the line ($0=$blank,
...
If you got lost in the terrarium don't worry!
Just use the \textsf{turtle\_status()} function: it returns
the current drawing settings. It tells you whether
the Turtle and its path are visible,
the width and height of the terrarium, where the Turtle is placed
right now and at which angle.

<<results=verbatim, eval=TRUE>>=
turtle_status()
@

If you just want to know where the Turtle is or at which angle it points
try \textsf{turtle\_getpos()} and \textsf{turtle\_getangle()} functions, respectively.
...
If you wish to place the Turtle back at the starting position
...
is designed to call more complicated plot expressions, because it automatically hides the Turtle before starting the operations that occurs in a faster proceed of plotting.
...
This section shows how to connect the functions
...
\subsection{\textsf{If}}
There are some cases when you would like to call
a function when some condition is fulfilled.
The \textsf{if} expression enables you to do it.
The syntax is the following: \textsf{if(condition) \{\}}.
When the \textsf{condition} is fulfilled the sequence of actions
you put between the curly bracket is called. Let's see an example.
...
As you can see the condition is fulfilled so the Turtle turns right.
What if you would like to perform some actions when the condition
is fulfilled and some other actions otherwise? There is an answer to it too.
After the closing curly braces you write \textsf{else \{\}}
and again between the brackets there are the functions you would like to call.
...
The last thing you should know while using this package is recursion.
It is a process of repeating actions in the self-similar pattern.
