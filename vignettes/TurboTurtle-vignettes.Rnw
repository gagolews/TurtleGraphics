...
\bigskip
{\large A. Cena, M. Kosinski, B. Zogala-Siudem}
...
The \textsf{TurboTurtle} package offers to R-users functionality of the famous ''turtle graphics'' from Logo educational programming language. The main idea standing behind it is to keep kids away from drugs and encourage children to learn programming and show that working with computer can be fun and creative.
...
turtle_init()
@
Now, we can move the Turtle forward, for example for a distance of 3 units, with the use of the \textsf{move\_forward()} function.
<<fig.keep='none', eval=FALSE>>=
move_forward(dist=3)
@
If you would like to change directions -- you may use \textsf{left()} or \textsf{right()} functions, which change the Turtle direction of a given angle. For example, we may turn 45 degrees in the right  direction. The results are presented in Figure~\ref{Fig:initMove}
<<fig.keep='none', eval=FALSE>>=
right(angle=45)
move_forward(dist=5)
@
...
\begin{figure}[hbt!]
\subfigure[Initialization of the Turtle]{
\includegraphics[width=2.3in]{figures/init.pdf}}
\subfigure[Moving forward ]{
\includegraphics[width=2.3in]{figures/move.pdf}}
\subfigure[Turning right]{
\includegraphics[width=2.3in]{figures/turn.pdf}}
\subfigure[Moving forward]{
\includegraphics[width=2.3in]{figures/turnMove.pdf}}
\caption{TurboTurtle}\label{Fig:initMove}
\end{figure}

\subsection{Visibility settings}

To unable the path from being drawn we can simply use \textsf{up()} function. Let us consider simple example. We will turn the Turtle in right direction about 90 degrees and then \textsf{up()} function. Now when we move forward the path is not visible. If we want to draw the path we should call \textsf{down()} function. 
...
Similarly, we may show or hide thr Turtle image, using \textsf{show()} and \textsf{hide()} functions respectively.
...
The results are depicted in Figure~\ref{Fig:UpDown}.
...
\end{figure}
$\,$\\
\noindent You may also change graphical paramaeters more easily by functions \textsf{set\_col()}, \textsf{set\_lty()}, \textsf{set\_lwd()}. Each of them takes exactly one argument to be passed to \textsf{set\_param()}.
